...
class MigrationGenerator
{
    ...
    public static function tableExists(string $tableName): bool
    {
        ...
        $database = $dicontainer->get(SystemConfig::class)->getConfigItem('db')['database'] ?? 'baseapi';
                ...
                foreach (scandir(__DIR__ . '/../src/model/' . $file) as $file2) {
                    if (is_file(__DIR__ . '/../src/model/' . $file . '/' . $file2)) {
                        ...
                        if (str_contains($fileContent, 'class ' . str_replace('.php', '', $file2))) {
                            $models[] = 'timanthonyalexander\BaseApi\\model\\' . $file . '\\' . str_replace('.php', '', $file2);
                        }
                    }
                }
            }
